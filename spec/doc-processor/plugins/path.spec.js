var plugin = require('../../../../lib/doc-processor/doc-processor-plugins/extract-meta-data/path');
describe("path doc processor plugin", function() {
  it("should compute the path from the document section and id if it is a js file", function() {
    ...
    plugin.each(doc);
    expect(doc.path).toEqual('/api/ng/directive/input[checkbox].html#someMethod');
  });
  ...
  it("should compute the path from the section file name and file type", function() {
    ...
    plugin.each(doc);
    expect(doc.path).toEqual('/guide/directives.html');
  });
});