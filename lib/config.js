 ...
 * @type Config
 * @param {Object|Config} [config] An optional object of initial properties to assign to the config
 *                                 object or a container Config object to which this config will have
 *                                 restricted access
 * @param {string} [propertyPath]  If the `config` param is an instance of Config then this should be
 *                                 a property path to a section of the containing config
 */
function Config(config, propertyPath) {
  if ( config instanceof Config ) {
    this.config = config.config;
    this.prependPath = propertyPath + '.';
  } else {
    this.config = {};
...
Config.prototype.applyTo = function(propertyPath, fn) {
  ...
  var parts = (this.prependPath + propertyPath).split('.');
  var property = parts.pop();
  var container = _.reduce(parts,
        function(obj, prop) { return obj[prop] || (obj[prop] = {}); },
        this.config
      );
 ...
 */
Config.load = function(configFilePath, config) {
  ...
  config.set('basePath', path.dirname(configFilePath));
