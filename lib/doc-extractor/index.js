...
module.exports = function fileReaderFactory(extractors) {
   ...
   * @param  {Array} files                      Array of glob patterns or objects containing a glob pattern and options
  ...
  return function(files) {
    ...
    return Q.all(_.map(files, function(fileInfo) {
      ...
      if ( _.isString(fileInfo) ) {
        basePath = path.dirname(fileInfo);
        files = glob.sync(fileInfo);
      } else if ( _.isObject(fileInfo) ) {
        basePath = fileInfo.options.cwd || process.cwd();
        files = glob.sync(fileInfo.pattern, fileInfo.options);
      } else {
