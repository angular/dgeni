...
var reader = {
   ...
   * @param  {object} defaults                      An object containing default properties for new docs
   * @return {promise}                              A promise to an array of docs that have been extracted
   */
  readDocs: function(path, pattern, extractDocFn, defaults) {
      ...
      files.forEach(function(file) {
        if ( pattern.test(file) ) {
          var docPromise = fs.read(file).then(function(content) {
            return extractDocFn(file, content, defaults);
          });
