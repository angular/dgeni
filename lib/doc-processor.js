...
function createChildInjector(baseInjector, docs, processor, config) {
  ...
  _.forEach(processor.config, function(_, propertyPath) {
    // We need to make the property path into a single JavaScript identifier for ease of injection
    module.value(propertyPath.replace('.', '_'), processorConfig.get(propertyPath));
  });
