...
PartialNames.prototype.addDoc = function(doc) {
  ...
  doc.partialNames = getPartialNames(doc.id);
...
PartialNames.prototype.getLink = function(url, title) {
  ...
  if ( _.isArray(doc) ) {
    ...
    linkInfo.error = 'Ambiguous link: "' + url + '".\n' +
      _.reduce(doc, function(msg, doc) { return '\n  "' + doc.id + '"'; }, 'Matching docs: ');
...
function getPartialNames(codeName) {
  return _getPartialNames(_parseCodeName(codeName));
}
...
module.exports = {
  ...
  getPartialNames: getPartialNames,
